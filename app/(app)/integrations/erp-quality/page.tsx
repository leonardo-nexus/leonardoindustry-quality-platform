...
import { ArrowLeftRight, AlertOctagon, CheckCircle2, RotateCw, Activity } from "lucide-react";
...
export default async function ErpQualityIntegrationPage() {
  ...
  const pending = (outbox ?? []).filter((o: any) => o.status === "pending").length;
  const failed = (outbox ?? []).filter((o: any) => o.status === "failed").length;
  const sentRecent = (outbox ?? []).filter((o: any) => o.status === "sent").length;
  ...
  return (
    <>
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <Kpi label="Conflitti aperti" value={conflicts?.length ?? 0} color="text-status-red" icon={ArrowLeftRight} />
      </div>
        ...
        <Card className="leo-card mb-6 border-2 border-status-red/40">
          <CardHeader>
            <CardTitle className="text-base text-status-red flex items-center gap-2">
              <AlertOctagon className="h-4 w-4" /> Conflitti da risolvere ({conflicts?.length})
            </CardTitle>
          </CardHeader>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <Card className="leo-card">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base"><RotateCw className="h-4 w-4" /> Outbox eventi ({outbox?.length ?? 0})</CardTitle>
          </CardHeader>
          <CardContent className="space-y-1 max-h-96 overflow-y-auto">
            ...
            {(outbox?.length ?? 0) === 0 && <p className="text-xs text-leo-muted">Nessun evento outbox</p>}
          </CardContent>
        </Card>
        ...
        <Card className="leo-card">
          ...
          <CardContent className="space-y-1 max-h-96 overflow-y-auto">
            ...
            {(recentLogs?.length ?? 0) === 0 && <p className="text-xs text-leo-muted">Nessun log</p>}
          </CardContent>
        </Card>
        ...
        <Card className="leo-card lg:col-span-2">
          <CardHeader>
            <CardTitle className="text-base">Mapping global_id ↔ ERP ↔ Quality ({mappings?.length ?? 0})</CardTitle>
          </CardHeader>
          <CardContent className="space-y-1">
            ...
            {(mappings?.length ?? 0) === 0 && <p className="text-xs text-leo-muted">Nessun mapping</p>}
          </CardContent>
        </Card>
        ...
        <Card className="leo-card lg:col-span-2 border-leo-border/50">
          ...
          <CardContent className="space-y-2 text-xs">
            <Endpoint method="POST" path="/api/integrations/erp/webhook" desc="ERP push aggiornamenti supplier (HMAC + timestamp + idempotency)" />
